...
func Test_Tcp_Start(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Minute, log).SpawnWorker(ctx, cmd)
...
func Test_Tcp_StartError(t *testing.T) {
	...
	w, err := serv.SpawnWorker(ctx, cmd)
	assert.Error(t, err)
	assert.Nil(t, w)
}
...
func Test_Tcp_Failboot(t *testing.T) {
	...
	w, err2 := NewSocketServer(ls, time.Second*5, log).SpawnWorker(ctx, cmd)
	assert.Nil(t, w)
	assert.Error(t, err2)
}
...
func Test_Tcp_Timeout(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Millisecond*1, log).SpawnWorker(ctx, cmd)
	assert.Nil(t, w)
	assert.Error(t, err)
...
func Test_Tcp_Invalid(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Second*1, log).SpawnWorker(ctx, cmd)
	assert.Error(t, err)
	assert.Nil(t, w)
}
...
func Test_Tcp_Broken(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Second*10, log).SpawnWorker(ctx, cmd)
	if err != nil {
		t.Fatal(err)
	}
	wg := sync.WaitGroup{}
...
func Test_Tcp_Echo(t *testing.T) {
	...
	w, _ := NewSocketServer(ls, time.Minute, log).SpawnWorker(ctx, cmd)
	go func() {
		assert.NoError(t, w.Wait())
	}()
	defer func() {
...
func Test_Unix_Failboot(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Second*5, log).SpawnWorker(ctx, cmd)
	assert.Nil(t, w)
	assert.Error(t, err)
}
...
func Test_Unix_Timeout(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Millisecond*100, log).SpawnWorker(ctx, cmd)
	assert.Nil(t, w)
	assert.Error(t, err)
	assert.True(t, errors.Is(errors.TimeOut, err))
}
