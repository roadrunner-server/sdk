...
func Test_Tcp_Start2(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Minute, log).SpawnWorker(context.Background(), cmd)
...
func Test_Tcp_StartCloseFactory2(t *testing.T) {
	...
	w, err := f.SpawnWorker(context.Background(), cmd)
	assert.NoError(t, err)
	assert.NotNil(t, w)
...
func Test_Tcp_Failboot2(t *testing.T) {
	...
	w, err2 := NewSocketServer(ls, time.Second*5, log).SpawnWorker(context.Background(), cmd)
	assert.Nil(t, w)
	assert.Error(t, err2)
}
...
func Test_Tcp_Invalid2(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Second*1, log).SpawnWorker(context.Background(), cmd)
	assert.Error(t, err)
	assert.Nil(t, w)
}
...
func Test_Tcp_Echo2(t *testing.T) {
	...
	w, _ := NewSocketServer(ls, time.Minute, log).SpawnWorker(context.Background(), cmd)
	go func() {
		assert.NoError(t, w.Wait())
	}()
	defer func() {
...
func Test_Unix_Failboot2(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Second*5, log).SpawnWorker(context.Background(), cmd)
	assert.Nil(t, w)
	assert.Error(t, err)
}
...
func Test_Unix_Timeout2(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Millisecond*100, log).SpawnWorker(context.Background(), cmd)
	assert.Nil(t, w)
	assert.Error(t, err)
...
func Test_Unix_Invalid2(t *testing.T) {
	...
	w, err := NewSocketServer(ls, time.Second*10, log).SpawnWorker(context.Background(), cmd)
	assert.Error(t, err)
	assert.Nil(t, w)
}
