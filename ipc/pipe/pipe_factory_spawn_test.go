...
func Test_GetState2(t *testing.T) {
	...
	w, err := NewPipeFactory(log).SpawnWorker(context.Background(), cmd)
	go func() {
...
func Benchmark_Pipe_SpawnWorker_Stop2(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		cmd := exec.Command("php", "../../tests/client.php", "echo", "pipes")
		w, _ := f.SpawnWorker(context.Background(), cmd)
		go func() {
			if w.Wait() != nil {
				b.Fail()
			}
...
func Benchmark_Pipe_Worker_ExecEcho2(b *testing.B) {
	...
	w, _ := NewPipeFactory(log).SpawnWorker(context.Background(), cmd)
